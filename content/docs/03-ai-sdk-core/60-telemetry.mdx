...
### Legacy AI SDK attributes on OpenTelemetry spans

Some observability platforms or existing dashboards may rely on the legacy AI SDK `ai.*` attributes. You can opt into those attributes while keeping the GenAI span structure by passing `legacyAttributes` to `OpenTelemetry`:
...
This does not create additional legacy spans such as `ai.generateText`, `ai.generateText.doGenerate`, or `ai.toolCall`. Instead, it adds the selected legacy attributes to the spans emitted by `OpenTelemetry`, such as `invoke_agent {modelId}`, `step {n}`, `chat {modelId}`, and `execute_tool {toolName}`.

You can also enable only specific legacy attribute groups. When an object is provided, all omitted groups default to `false`:
...
- `operation`: legacy operation/resource attributes such as `operation.name`, `resource.name`, `ai.operationId`, `ai.model.*`, and legacy GenAI request attributes.
- `settings`: `ai.settings.*` and legacy request setting attributes.
- `runtimeContext`: `ai.settings.context.*`.
- `requestHeaders`: `ai.request.headers.*`.
- `prompts`: prompt, message, tool definition, tool choice, document, and value input attributes.
- `schemas`: object generation schema attributes.
- `responses`: response text, object, files, tool calls, finish reason, response id/model/timestamp, and provider metadata.
- `usage`: `ai.usage.*`.
- `toolCalls`: `ai.toolCall.*`.
- `streaming`: `ai.stream.*`.
- `embeddings`: embedding input/output attributes.
- `reranking`: ranking input/output attributes.

The `recordInputs` and `recordOutputs` telemetry options are still respected for legacy attributes.
